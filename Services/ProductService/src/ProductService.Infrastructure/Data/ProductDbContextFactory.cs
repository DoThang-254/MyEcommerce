...
public class ProductDbContextFactory : IDesignTimeDbContextFactory<ProductDbContext>
{
    public ProductDbContext CreateDbContext(string[] args)
    {
        ...
        var apiPath = Path.GetFullPath(Path.Combine(currentDirectory, "src", "ProductService.API"));

        // Kiểm tra nếu không thấy thư mục API (trường hợp bạn đang đứng ở bên trong src)
        if (!Directory.Exists(apiPath))
        {
            apiPath = Path.GetFullPath(Path.Combine(currentDirectory, "..", "ProductService.API"));
        }
