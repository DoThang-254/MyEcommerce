...
public class OrderDbContextFactory : IDesignTimeDbContextFactory<OrderDbContext>
{
    public OrderDbContext CreateDbContext(string[] args)
    {
        ...
        var apiPath = Path.GetFullPath(Path.Combine(currentDirectory, "src", "OrderService.API"));

        // Kiểm tra nếu không thấy thư mục API (trường hợp bạn đang đứng ở bên trong src)
        if (!Directory.Exists(apiPath))
        {
            apiPath = Path.GetFullPath(Path.Combine(currentDirectory, "..", "OrderService.API"));
        }
        ...
        return new OrderDbContext(builder.Options, mockMediator.Object, mockCurrentUserService.Object);
    }
}